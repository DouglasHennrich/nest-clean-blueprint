...
> **Arquitetura de referência bootstrap para novos backends NestJS.**
> Arquitetura em camadas estrita, tratamento de erros baseado em Result, I/O validado com Zod, respostas moldadas por presenters e providers compartilhados (mail, encrypt-decrypt, upload). Inclui um servidor MCP para que agentes de IA possam sempre consultar as diretrizes mais recentes.
...
```bash
git clone https://github.com/<owner>/nest-clean-blueprint.git minha-nova-api
cd minha-nova-api
rm -rf .git && git init
pnpm install
...
```json
{
  "mcpServers": {
    "nest-clean-blueprint": {
      "command": "node",
      "args": ["/caminho/absoluto/para/nest-clean-blueprint/mcp-server/dist/index.js"]
    }
  }
}
```
