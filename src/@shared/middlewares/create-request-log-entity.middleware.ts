...
export class CreateRequestLogEntityMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    res.on('finish', () => {
      ...
      const files =
        earlyRequestData.files ?? Sanitize.files((req as any).files);
      ...
      const payload = {
        ...
        userName:
          (currentUser?.name as string) ?? (user?.name as string) ?? undefined,
        userType:
          (currentUser?.userType as string) ??
        ...
        requestId: AsyncContext.getRequestId() ?? earlyRequestData.requestId,
      ...
      this.requestLogFlushSchedulerService.enqueue(payload).catch((err) => {
        this.logger.error(
          `Failed to enqueue request log: ${(err as Error).message}`,
        );
      });
    });
